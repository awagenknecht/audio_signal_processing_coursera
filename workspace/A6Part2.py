...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    start, end = 0, 0
    while end < le-1:
        if stable[end+1] == (stable[end] + 1):
            end += 1
            continue
        segments.append([stable[start], stable[end]])
        start = end + 1
        end += 1
    segments.append([stable[start], stable[end]])

    #5. apply segment filtering, i.e. remove segments with are < minNoteDur in length
    segments = list(filter(lambda x: x[1] - x[0] + 1 >= minNoteDur*fs/H, segments))
