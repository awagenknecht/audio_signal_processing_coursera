...
def suppressFreqDFTmodel(x, fs, N):
    ...
    # use dftAnal to obtain the spectrum
    mX, pX = dftAnal(x, w, N)

    # use dftSynth to perform inverse transform without filtering
    y = dftSynth(mX, pX, len(w)) * outputScaleFactor 

    # filter out frequencies <= 70 Hz from magnitude spectrum
    k = int(np.ceil(N * 70 / fs)) # calculate nearest bin >= 70 Hz
    mX[:k+1] = -120 # all bins <= k set to -120 dB
    
    # use dftSynth to perform inverse transform with the filtered magnitude spectrum
    yfilt = dftSynth(mX, pX, len(w)) * outputScaleFactor
