...
# from A2.A2Part1 import genSine
"""
...
def zpFFTsizeExpt(x, fs):
    ...
    # (Implicitly does a zero-padding of xseg by 256 samples)
    mX3 = dftAnal(xseg, w1, 2*M)[0]

    ## plotting setup

    # freq_axis = fs * np.arange(len(mX2)) / (2*M)

    # plt.plot(freq_axis[::2], mX1)
    # plt.plot(freq_axis, mX2)
    # plt.plot(freq_axis, mX3)
    # plt.show()
    # print(np.argmax(mX1), np.argmax(mX2), np.argmax(mX3))

    mX1_80, mX2_80, mX3_80 = mX1[:80], mX2[:80], mX3[:80]
    return (mX1_80, mX2_80, mX3_80)

# x = genSine(0.8, 110.0, 0.0, 1000.0, 512/1000)
# mX1_80, mX2_80, mX3_80 = zpFFTsizeExpt(x, 1000)