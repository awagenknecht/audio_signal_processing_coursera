...
def exploreSineModel(inputFile='../../sounds/multiSines.wav'):
    ...
    # fftbins=False due to odd window length
    w = get_window(window, M, fftbins=False)                   # compute analysis window
    ...
    # calculate SNR
    # # match size of y to size of x with interpolation
    # y_interp = interp1d(np.arange(len(y)), y, kind='quadratic')
    # y_stretched = y_interp(np.linspace(0, len(y)-1, len(x)))
    # # find the noise
    # noise = y_stretched - x
    # noise[noise < eps] = eps
    minLen = min(len(x), len(y))
    err = np.sum((x[:minLen] - y[:minLen]) ** 2) / minLen
    print(f"MSE: {err}")
    # # case 1 (whole signal)
