...
def minFreqEstErr(inputFile, f):
    ...
    # analysis parameters:
    window = 'blackman'
    t = -40 # peak-picking magnitude threshold in dB
    
    # load input file
    fs, x = UF.wavread(inputFile)

    # window sizes to try
    M_range = [100*x + 1 for x in range(1,22)]
    ...
    for M in M_range:
        ...
        # perform DFT analysis on portion of signal same length as window
        mid = fs // 2 # 0.5 seconds into the file
        mX, pX = DFT.dftAnal(x[mid-M//2:mid+1+M//2], w, N)

        # find peak and perform interpolation
        ploc = UF.peakDetection(mX, t)
        iploc, ipmag, ipphase = UF.peakInterp(mX, pX, ploc)

        # estimate frequency (there should only be one peak in mX, so only one value in iploc)
        fEst = iploc[0] * fs / N
        # print(M, N, fEst, abs(fEst - f))
