...
def optimalZeropad(x, fs, f):
    ...
    """
    # Find length of input, M, and length of a period in samples
    M = len(x)
    period = int(fs / f)
    # Find least integer multiple of period that is greater than M
    N = M + period - (M % period)
    # Compute the size N DFT (zero-padding is done implicitly)
    X = fft(x, N)
    # Compute and return positive half of magnitude spectrum
    absX = abs(X[:(N//2)+1])  # compute ansolute value of positive side
