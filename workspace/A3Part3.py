...
def testRealEven(x):
    ...
    """
    # Perform zero phase windowing (works for even or odd input length)
    M = len(x)
    hM1 = (M+1)//2
    hM2 = M//2
    dftbuffer = [0] * M
    ...
    # Calculate the M-point DFT
    X = fft(dftbuffer)

    # Calculate magnitude spectrum and imaginary part of DFT
    mX = abs(X)
    iX = np.imag(X)

    # Check if input x is real and even using properties of the DFT
    # Check that imaginary part of DFT is 0 and magnitude spectrum is even
    # within 1e-6 tolerance
    # exclude 0th index (DC component) when checking even-ness of magnitude
    isRealEven = max(abs(iX)) < 1e-6 and max(abs(mX[1:hM1] - mX[-1:-hM1:-1])) < 1e-6

    return (bool(isRealEven), np.array(dftbuffer), X)
