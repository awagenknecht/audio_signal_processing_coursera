...
def mainlobeTracker(inputFile = '../../sounds/sines-440-602-hRange.wav'):
    ...
    # Since the high-frequency component has an amplitude of 2e-3, we need a threshold of at least -54 dB
    # to find the high-frequency mainlobe (20*log10(2/1000) = -53.979)
    # Therefore, we need a window where the highest sidelobe has magnitude less than -54 dB to avoid tracking
    # the low-frequency sidelobes
    ...
    w = get_window(window, M, fftbins=fftbins_bool)                   # Compute analysis window
    ...
    plt.plot(tStamps,fTrackEst[:,0], color = 'y', linewidth=2.0)
    plt.plot(tStamps,fTrackEst[:,1], color = 'g', linewidth=2.0)
...
# w_range = ['boxcar', 'hann', 'hamming', 'blackman', 'blackmanharris']
# t_range = [-10, -20, -30, -40, -50, -60, -70, -80]
