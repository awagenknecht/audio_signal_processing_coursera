...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    """
    # Calculate smallest positive integer for which the positive half of the DFT magnitude 
    # spectrum has only two non-zero values
    p1, p2 = int(fs/f1), int(fs/f2) # find the periods in number of samples
    M = int(p1 * p2 / gcd(p1,p2))

    # take DFT of first M samples of input
    X = fft(x, M)

    # consider only positive half of the DFT and find magnitude in decibels
