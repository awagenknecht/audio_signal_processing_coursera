...
def extractMainLobe(window, M):
    ...
    # use fftshift to center main lobe around middle sample, not around 0
    mX = abs(fftshift(fft(w, N)))
    mX[mX < eps] = eps # add small value to avoid taking log of 0
    mX = 20*np.log10(mX) # convert to dB

    # find first local minimum to the right of center
    m = N // 2
    i = 0
    while mX[m+i+1] < mX[m+i]:
        i += 1

    # return main lobe of mX between local minima
    return mX[m-i:m+i+1]
