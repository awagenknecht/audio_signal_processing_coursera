...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
    ...
    w  = get_window(window, M, fftbins=fftbins_bool)            #obtaining analysis window    
    ...
    frame1, frame2 = int(np.round(t1 * fs / H)), int(np.round(t2 * fs / H))
    xhfreq_seg, xhmag_seg = xhfreq[frame1:frame2+1], xhmag[frame1:frame2+1]
    
    # 3. Compute the mean inharmonicity of the segment
    inharm = np.zeros(len(xhfreq_seg))

    for i in range(len(xhfreq_seg)): # iterate over all frames in time segment
        r = nH # keep track of number of detected harmonics at each frame
        for j in range(1, nH): # iterate over number of harmonics
            if xhfreq_seg[i, j] < eps: # no harmonic detected
                r -= 1 # decrement number of detected harmonics
                continue
            inharm[i] += abs(xhfreq_seg[i, j] - (j+1) * xhfreq_seg[i, 0]) / (j+1)
        inharm[i] = inharm[i] / r if r > 0 else None

    meanInharm = np.mean(inharm)
...
# meanInharm = estimateInharmonicity(inputFile, t1, t2, window, M, N, H, f0et, t, minf0, maxf0, nH)
# print(meanInharm)
